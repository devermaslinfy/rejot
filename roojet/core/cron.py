...
from roojet.users.models import User#, Customer
from roojet.services.utils import get_historic



def calculate_expected_improvement(user, orders, product_id=None,
                                   variable='revenue'):
    """
    ...
    dates = []
    items = {}
    get_historic(user=user, orders=orders, product_id=product_id)
...
def calculate_expected_improvement_list(user, orders, items=[],
                                        variable='revenue'):
    ...
    for item in items:
        output[item] = calculate_expected_improvement(
            user, orders, item, variable)
    return output


# def update_shopify_price(user, price, variant_id):
#     session = shopify.Session(user.shop_name + ".myshopify.com",
#                               user.shop_token)
#
...
class MyCronJob(CronJobBase):
    ...
    def do(self):
        ...
        for user in users:
            #try:
                #if user.userplan.expire < datetime.datetime.today():
                    #customer_obj = Customer.objects.get(user=user)
                    #stripe.api_key = "sk_live_V4Uj8MdAqwQgomKKSZfUddes"
                    #customer = stripe.Customer.retrieve(customer_obj.customer)
                    #customer.subscriptions.create(plan=user.userplan.plan.name)
            #except:
                #pass
            session = shopify.Session(user.shop_name + ".myshopify.com",
                                 user.shop_token)
            ...
            if orders:
                ...
                calculate_expected_improvement_list(user, orders,
