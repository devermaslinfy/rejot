    ...
    for product in products:
        ...
        for variant in product.attributes['variants']:
            ...
            if variant.attributes['title'] != 'Default Title':
                title += "/" + str(variant.attributes['title'])
            if not Product.objects.filter(created_by=request.user,
                                          shopify_variant_id=variant_id):
                try:
                    ProductValidator()(request.user, add=1)
                except ValidationError:
                    max_product_reached = True
                    return Product.objects.filter(
                        created_by=request.user), max_product_reached

                Product.objects.create(created_by=request.user,
                ...
                Product.objects.filter(shopify_variant_id=variant_id).update(
                                       ...
                                       actual_shopify_price=price,
                                       number_of_sells=sells)
    return Product.objects.filter(created_by=request.user), max_product_reached


def get_historic(user, orders, product_id):
